...
app.post('/grpc/rooms', (req, res) => {
  ...
  RoomService.createRoom({ numero, type, status, prix, description }, async (err, response) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
    await sendRoomMessage('creation', { id: response.room.id, numero, type, status, prix, description });
...
app.get('/grpc/rooms/:id', (req, res) => {
  ...
  RoomService.getRoom({ room_id: roomId }, async (err, response) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
    await sendRoomMessage('recuperation', { roomId });
    res.json(response.room);
  });
});
...
app.put('/grpc/rooms/:id', (req, res) => {
  const roomId = req.params.id;
  const { numero, type, status, prix, description } = req.body;
  RoomService.updateRoom({ room_id: roomId, numero, type, status, prix, description }, async (err, response) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
    await sendRoomMessage('modification', { id: roomId, numero, type, status, prix, description });
    res.json(response.room);
  });
});

app.get('/grpc/rooms', (req, res) => {
  RoomService.getAllRooms({}, async (err, response) => {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
...
app.delete('/grpc/rooms/:id', (req, res) => {
  ...
  RoomService.deleteRoom({ room_id: roomId }, async (err, response) => {
    ...
    try {
      ...
      await sendRoomMessage('suppression', { id: roomId });
...
app.get('/grpc/reservation/:id', (req, res) => {
  ...
  ReservationService.getReservation({ reservation_id: reservationId }, async (err, response) => {
    if (err) {
      res.status(500).send("Error while fetching reservation: " + err.message);
      return;
    }
    await sendReservationMessage('recuperation', { reservationId });
...
app.post('/grpc/reservation', async (req, res) => {
  try {
    ...
    await sendReservationMessage('creation', { id: reservation._id, client, room, dateStart, dateEnd });
...
app.put('/grpc/reservation/:id', (req, res) => {
  const reservationId = req.params.id;
  const { client, room, dateStart, dateEnd } = req.body;
  ReservationService.updateReservation({ reservation_id: reservationId, client, room, dateStart, dateEnd }, async (err, response) => {
    if (err) {
      res.status(500).send("Error while updating reservation: " + err.message);
      return;
    }
    await sendReservationMessage('modification', { id: reservationId, client, room, dateStart, dateEnd });
    res.json(response.reservation);
  });
});
...
app.delete('/grpc/reservation/:id', async (req, res) => {
  ...
  try {
    ...
    await sendReservationMessage('suppression', { id: reservationId });
...
app.post('/room', async (req, res) => {
  try {
    ...
    await sendRoomMessage('creation', { id: room._id, numero, type, status, prix, description });
    res.json(room);
  } catch (err) {
...
app.put('/room/:id', async (req, res) => {
  try {
    ...
    await sendRoomMessage('modification', { id: updatedRoom._id, numero, type, status, prix, description });
...
app.delete('/room/:id', async (req, res) => {
  try {
    ...
    await sendRoomMessage('suppression', { id: req.params.id });
...
app.get('/room/:id', async (req, res) => {
  try {
    ...
    res.json(room);
    await sendRoomMessage('recuperation', { roomId: req.params.id });
  } catch (err) {
...
app.delete('/reservation/:id', async (req, res) => {
  try {
    ...
    room.status = 'libre';
    await room.save();
    await sendReservationMessage('suppression', { id: reservation._id });
    res.json({ message: "Reservation deleted successfully" });
  } catch (err) {
    res.status(500).send("Error while deleting reservation: " + err.message);
  }
...
app.put('/reservation/:id', async (req, res) => {
  try {
    ...
    await sendReservationMessage('modification', { id: updatedReservation._id, client, room, dateStart, dateEnd });
...
app.get('/reservation/:id', async (req, res) => {
  try {
    ...
    res.json(reservation);
    await sendReservationMessage('recuperation', { reservationId: req.params.id });
  } catch (err) {
    res.status(500).send("Error while fetching reservation: " + err.message);
  }
});
